...
class KeplerianDisk(ThinDisk):
    ...
    def __init__(
        ...
        fd_scale = (
            3 * mass_dot_si / 64 / math.pi / cs_scale
        )  # This needs re-evaluation of its derivation, looks simplified.
        ...
        self._sf_scale = sf_scale.to(
            self.units
        ).f  # Surface flux scale in geometric units
        self._cs_scale = cs_scale.to(
            self.units
        ).f  # Sound speed scale in geometric units
        ...
        self._sd_scale = (
            sd_scale.to(self.units).f / self._vsc_scale
        ...
        self._kes_scale = (
            kappa_es * (bhU.area / bhU.mass).to(self.units).f
        )  # Thomson opacity scale
    ...
    def temperature(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (1 - (self.r_in / r_masked).sqrt()).pow(0.25) * (
            r_masked / 2.0
        ).pow(-0.75)
        return outp
    ...
    def sound_speed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (
            self._cs_scale
            * (r_masked / 2.0).pow(-0.375)
            * (1 - (self.r_in / r_masked).sqrt()).pow(0.125)
        )
        return outp
    ...
    def viscosity(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (
            self._vsc_scale
            * (r_masked / 2.0).pow(0.75)
            * (1 - (self.r_in / r_masked).sqrt()).pow(0.25)
        )
        return outp
    ...
    def flux_density(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (
            self._fd_scale
            * (r_masked / 2.0).pow(-4.125)
            * (1 - (self.r_in / r_masked).sqrt()).pow(0.875)
        )
        return outp
    ...
    def surface_density(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (
            self._sd_scale
            * (r_masked / 2.0).pow(-0.75)
            * (1 - (self.r_in / r_masked).sqrt()).pow(0.75)
        )
        return outp
    ...
    def rest_mass_density(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outp[mask] = (
            self._rm_scale
            * (r_masked / 2.0).pow(-1.5)
            * (1 - (self.r_in / r_masked).sqrt()).pow(0.625)
        )
        return outp
